...
class SnakeEnv(gym.Env):
    def __init__(self, seed=0, board_size=12, silent_mode=True, limit_step=True):
        ...
        self.action_space = gym.spaces.Discrete(4) # 动作空间，四个方向，离散的
        
        self.observation_space = gym.spaces.Box(
            low=0, high=255,
            shape=(84, 84, 3),
            dtype=np.uint8
        )# 观察空间，84*84的图像，RGB三通道，无符号8位整数
        ...
        self.done = False #game里面的结束标志

        if limit_step:
            self.step_limit = self.grid_size * 3 # More than enough steps to get the food.#限制步数
        else:
    ...
    def step(self, action):
        ...
        if info["snake_size"] == self.grid_size: # Snake fills up the entire board. Game over.
            reward = self.max_growth * 0.1 # 长度的奖励值
            self.done = True
        ...
        if self.done: # Snake bumps into wall or itself. Episode is over.
            ...
            reward = - math.pow(self.max_growth, (self.grid_size - info["snake_size"]) / self.max_growth) # (-max_growth, -1)  长度的惩罚值 越接近最长长度越好          
            reward = reward * 0.1
            return obs, reward, self.done, info
            ...
            if np.linalg.norm(info["snake_head_pos"] - info["food_pos"]) < np.linalg.norm(info["prev_snake_head_pos"] - info["food_pos"]):# 距离靠近食物，获得奖励，长度越小，奖励越大  nrom是计算向量的长度，默认参数即欧式距离
    ...
    def _generate_observation(self):
        obs = np.zeros((self.game.board_size, self.game.board_size), dtype=np.uint8) # Create an empty 2D array of size board_size x board_size.
