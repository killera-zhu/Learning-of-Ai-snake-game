...
class SnakeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.done: 
            reward = (- math.pow(self.max_growth, (self.grid_size - info["snake_size"]) / self.max_growth))*0.8
            return obs, reward * 0.1, self.done, info
        
        elif info["food_obtained"]:
            reward = 1 - math.exp((self.reward_step_counter - self.grid_size) / self.grid_size)
            self.reward_step_counter = 0 
